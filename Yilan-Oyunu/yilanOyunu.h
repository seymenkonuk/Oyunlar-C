...
// mod: 0 -> Yýlan Duvara Çarparsa Ölür
// mod: 1 -> Yýlan Duvara Çarparsa Ters Tarafdan Çýkar
...
int oynaYilanOyunu(int genislik, int yukseklik, int elmaSayisi, int mod) {
    ...
    gotoxy(yilan[0][0]*2, yilan[0][1]); printf("%c%c", 219, 219);
    ...
    for (i=1; i<=elmaSayisi; i++) {
        ...
        gotoxy((elma_x+1)*2, elma_y+1); printf("%c", 254);
    }
